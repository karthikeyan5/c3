...
- **`/status` bot command**: registered at startup via `setMyCommands`, so it autocompletes in Telegram's `/` menu. An inbound whose text is `/status` (or `/status@<botname>`, case-insensitive, exact token) is intercepted in the poll path **before gating/routing** — the broker answers it directly and the update is **never queued or routed to an agent** (its `update_id` is marked done so the offset can advance past it). This is a distinct surface from the `/c3:status` CLI slash command. The dispatcher is deliberately tiny — only `/status` is wired today; `/drain`/`/clear` are trivially addable later (YAGNI for now). In a topic it answers that topic's queue depth + attach state; in DM/General it answers a broker-wide summary.
