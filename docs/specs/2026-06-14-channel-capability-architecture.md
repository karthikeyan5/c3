...
- v5 — **built (P0–P7) + triple-reviewed (2026-06-15).** Intentional divergences recorded: the P1 `parse_mode` back-compat shim was dropped outright in P7 (single coherent build, no live sessions to migrate; `Markup=native` remains a dormant, non-agent-facing escape hatch). Caption over-limit is a clean **pre-send error** in v1 (gate trim+follow-up deferred). Inline-emphasis split across a chunk boundary degrades to literal markers (block constructs + links are protected; documented limitation). Post-review fixes applied: typing-relay idle-disarm (was pulsing forever once armed if the agent stopped replying), italic-wrapping-bold + link-balanced-paren converter bugs, converted-caption-length check, download-cap pre-check, and a broker-side audit log of every media `path` send (secret-exfil detectability under the trusted-local-agent model).
