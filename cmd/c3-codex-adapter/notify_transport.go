...
// method names. Used for `notifications/message` log frames in the Codex
// adapter (the SDK's typed Log API enforces level filtering and would
...
//
// Mirrors cmd/c3-claude-adapter/notify_transport.go — same rationale,
...
//
// The adapter does NOT call this today: the SDK's transports do not
// transparently reconnect, and the adapter restarts on a dropped
// transport. Preventative scaffolding for a future SDK version that
// adds transparent reconnect — analogous to the Claude adapter's
// notifyTransport.Disconnect. Closes report MINOR m2 (2026-05-19).
