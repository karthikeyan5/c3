...
- **STT pipeline step-timing instrumentation** — *maintainer wants; do first of the STT-latency pair.* STT feels laggy. The voice file makes several hops: phone → Telegram servers → download to the host → upload to the STT API → transcript back → echo to phone + agent. Log a timestamp at each hop so that, over several days of real use, there is per-stage analytics to decide where the latency actually is before optimizing. Instrument first, optimize second.
- **STT latency — eliminate the host round-trip** — *maintainer wants; gated on the timing data above.* Hypothesis: downloading the voice file to the host and re-uploading it to the STT API is the dominant cost. Options to cut it: (a) a self-hosted Telegram Bot API server that exposes a direct file URL, passed to the STT API *as a file* (not as a link inside the prompt); (b) if the standard Bot API can hand the file straight to the STT API server-side, prefer that (no new infra); (c) worst case, run a thin STT segment on a maintainer-owned server with better network locality that downloads the file and calls the API, returning only the transcript — keep everything else (prompt, provider config, fallback chain) on the host so day-to-day updates stay easy. The STT package is already standalone Python, so a server-side install can be scripted/documented. Decide the approach after the timing data lands.
