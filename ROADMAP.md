...
- **rich_message inbound decode shim** — `planned` (Karthi: build next round) — forwarded Bot-API-10.1 rich messages carry the body in `Message.rich_message`, which gotgbot rc.34 silently drops → empty `.Text`. Increasingly Telegram will send rich messages and we must not surface empty messages. Fix = augmented `getUpdates` decode shim parsing `rich_message` blocks → text (works on rc.34, no dep bump). Design: `RESUME.md` §FIX1 + research outputs w4ocivw74/wb63isan9.
