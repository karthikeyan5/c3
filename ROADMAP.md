...
- **Rich-message inbound decode** — `done` — Bot-API-10.1 rich messages arrive in `Message.rich_message` (a recursive `RichBlock`/`RichText` tree) which gotgbot rc.34 silently drops → empty `.Text`. The poll loop now captures the raw `rich_message` JSON (raw `getUpdates` + a probe unmarshal, verified byte-equivalent to gotgbot's own `GetUpdates`) and a new decoder (`internal/channel/telegram/richdecode.go`) renders the tree → GFM markdown (headings, emphasis, links, lists, blockquotes, **tables**, code/math) + **media blocks → downloadable attachments**. Hard invariants: never empty when a rich message is present (`[rich message]`/`[unsupported block:…]` markers), never panics the poll loop (`recover()`). Per-channel `rich_inbound` toggle (default on; **broker restart to change** — startup snapshot, not SIGHUP) + `DeliversRichMessages` capability flag. Built TDD across 8 tasks; per-task + broad whole-branch review clean (untrusted-input recursion risk empirically disproven — Go's JSON decoder errors out before any stack overflow). Spec: [`docs/superpowers/specs/2026-06-19-c3-rich-message-inbound-decode-design.md`](docs/superpowers/specs/2026-06-19-c3-rich-message-inbound-decode-design.md); plan: [`docs/superpowers/plans/2026-06-19-rich-message-inbound.md`](docs/superpowers/plans/2026-06-19-rich-message-inbound.md).
  - **Deferred follow-ups (non-blocking, Karthi's call):** (1) defensive depth-counter in `renderBlocks`/`renderRichText` as defense-in-depth on untrusted input (review said not needed for merge); (2) thin test-coverage nits — direct `recover()`-path test, full `escapeInline` char-set test, ragged-row table test, assert `DeliversRichMessages` in the golden-manifest test.
...
- **Formatting policy — make agents format liberally** — `planned` (Karthi 2026-06-19: "anything that makes a wall of text easier to read should be done; no reason to keep it plain"). The converter bug is now fixed; this is the agent-guidance half. Implement: (1) reframe the RichText line in `internal/capability/guidance.go` from permissive → prescriptive; (2) add a formatting nudge to the `reply` tool Description in BOTH adapters (`cmd/c3-claude-adapter/main.go`, `cmd/c3-codex-adapter/main.go`) — the compose-time surface, currently silent on formatting; (3) reorder `internal/mode/protocol.go` `Combined()` so CHANNEL CAPABILITIES isn't dead-last under the suppression-heavy mode protocol; (4) add one worked formatted-reply example; (5) update the agent plain-prose memory (`feedback_telegram_mode.md`) to carve out structural content (code/tables/lists/quotes/links). Root-cause: workflow wyqdrd011 / `.git/sdd/richtext-findings.md`.
...
- **FIX #1 (parked): inbound delivery-drop + album/media-group drop** — `partially resolved`
  ...
  **Still open (back-to-back TEXT half):** the historical "msgs 186/187 ~33µs apart, only one reached the agent" symptom is unconfirmed — two text messages within the 1.5s window are debounce-MERGED into one block (texts joined with `\n`), which can *look* like one message but contains both. NEXT: confirm whether that case is a genuine drop or just merge-perception before doing more work.
