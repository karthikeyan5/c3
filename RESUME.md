...
1. **`c3-broker release <cwd>`** runtime IPC op (currently stubbed). Lets
   a project free its attached topic without restarting the broker.
2. **Phase 3 (access control)** — pairing, master user enforcement.
3. **Phase 4 items** — pick from `TODO.md` against value/effort.
